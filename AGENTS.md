...
LoongClaw is a Rust multi-channel agent runtime for Telegram, Discord, and Web.
...
- `loongclaw-core`: shared error/types/text (`error`, `llm_types`, `text`)
- `loongclaw-storage`: SQLite DB, memory domain, usage report assembly
- `loongclaw-tools`: tool runtime primitives, sandbox, path guards, web/todo helpers
- `loongclaw-channels`: channel abstractions (`channel`, `channel_adapter`, delivery boundary)
- `loongclaw-app`: app-level support modules (logging, builtin skills, transcribe)

## Tool system

`src/tools/mod.rs` assembles built-in tools, while shared runtime primitives live in `loongclaw-tools::runtime`:
...
- Default skills dir: `<data_dir>/skills` (default `data_dir` is `~/.loongclaw`)
- Config override: `skills_dir` in `loongclaw.config.yaml` (e.g. `~/.loongclaw/skills`)
- Compatibility policy: existing configured `data_dir` / `skills_dir` / `working_dir` keep working; new defaults apply only when unset
- ClawHub lockfile: `~/.loongclaw/clawhub.lock.json`
...
- For the separate `website` repository (`loongclaw.github.io`), do not open a PR by default; commit/push directly unless explicitly requested otherwise.
