...
LoongClaw is a Rust multi-platform chat bot with a channel-agnostic core and platform adapters. It currently supports Telegram, Discord, Slack, Feishu/Lark, and Web, and can be extended to more platforms. It provides agentic tool execution, web search, scheduled tasks, and persistent memory. Inspired by [nanoclaw](https://github.com/gavrielc/nanoclaw/) (TypeScript/WhatsApp), incorporating some of its design ideas.
...
- `web/` -- Built-in Web UI (React + Vite). Compiled to `web/dist/` and embedded into the Rust binary via `include_dir!`. This is the chat interface and settings panel served by loongclaw itself at runtime.
- `website/` -- **Separate git repository** (landing page + documentation site). Not part of the loongclaw binary. Contains the public-facing marketing site and docs. Changes here have no effect on the bot.

## Project layout

- `crates/loongclaw-core/` -- shared error/types/text modules (`error`, `llm_types`, `text`)
- `crates/loongclaw-storage/` -- SQLite DB schema/query layer + memory/usage domain modules
- `crates/loongclaw-tools/` -- tool runtime primitives (trait/auth/risk/schema/path) + sandbox
- `crates/loongclaw-channels/` -- channel abstraction and delivery boundary modules
- `crates/loongclaw-app/` -- app-level support modules (logging, builtin skills, transcribe)
...
LoongClaw uses `loongclaw.config.yaml` (or `.yml`) for configuration. Override the path with `LOONGCLAW_CONFIG` env var. See `loongclaw.config.example.yaml` for all available fields.

## Soul (personality customization)

LoongClaw supports a `SOUL.md` file that defines the bot's personality, voice, values, and working style. The file content is injected into the system prompt, replacing the default "helpful AI assistant" identity.
...
Core persistence is provided by `loongclaw-storage` (`Database` wrapper over SQLite). Runtime state and observability tables are managed through versioned migrations.
