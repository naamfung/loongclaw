...
TAP_REPO="loongclaw/homebrew-tap"
FORMULA_PATH="Formula/loongclaw.rb"
GITHUB_REPO="loongclaw/loongclaw"
...
BINARY="target/release/loongclaw"
if [ ! -f "$BINARY" ]; then
...
# --- Create tarball ---
ARCH="$(detect_arch)"
TARBALL_NAME="loongclaw-$NEW_VERSION-${ARCH}-apple-darwin.tar.gz"
TARBALL_PATH="target/release/$TARBALL_NAME"

tar -czf "$TARBALL_PATH" -C target/release loongclaw
