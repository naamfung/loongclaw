...
WORK_DIR="${MATRIX_SMOKE_DIR:-/tmp/loongclaw-matrix-smoke}"
PROJECT_NAME="${MATRIX_SMOKE_PROJECT:-loongclaw-matrix-smoke}"
...
LOG_FILE="$WORK_DIR/loongclaw.log"
CONFIG_FILE="$WORK_DIR/loongclaw.matrix-smoke.yaml"
STATE_FILE="$WORK_DIR/state.env"
RUNTIME_ROOT="$WORK_DIR/loongclaw.data"
DB_PATH="$RUNTIME_ROOT/runtime/loongclaw.db"
...
cleanup() {
  if [[ -n "${LOONGCLAW_PID:-}" ]]; then
    kill "$LOONGCLAW_PID" >/dev/null 2>&1 || true
    wait "$LOONGCLAW_PID" >/dev/null 2>&1 || true
  fi
...
if [[ "$BOT_TOKEN" == "null" || -z "$BOT_TOKEN" ]]; then
  echo "Failed to log in bot user" >&2
  exit 1
fi
if [[ "$ALICE_TOKEN" == "null" || -z "$ALICE_TOKEN" ]]; then
  echo "Failed to log in alice user" >&2
  exit 1
fi
...
ROOM_ID="$(curl -sS -X POST "$BASE_URL/_matrix/client/v3/createRoom" \
  ...
  -d '{"name":"loongclaw-matrix-smoke","preset":"private_chat","invite":["@bot:localhost"]}' | jq -r '.room_id')"
...
LOONGCLAW_CONFIG="$CONFIG_FILE" cargo run --bin loongclaw -- start >"$LOG_FILE" 2>&1 &
LOONGCLAW_PID=$!
popd >/dev/null
