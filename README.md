...
Checks include PATH, shell runtime, browser executable, PowerShell policy (Windows), and MCP command dependencies from `<data_dir>/mcp.json` plus `<data_dir>/mcp.d/*.json`.
...
sqlite3 <data_dir>/runtime/loongclaw.db "SELECT id, chat_id, chat_channel, external_chat_id, category, embedding_model FROM memories ORDER BY id DESC LIMIT 20;"
```
...
<p align="center">
  <img src="docs/assets/readme/loongclaw-architecture.svg" alt="LoongClaw architecture overview" width="96%" />
</p>
...
<p align="center">
  <img src="docs/assets/readme/memory-architecture.svg" alt="LoongClaw memory architecture diagram" width="92%" />
</p>

LoongClaw maintains persistent memory via `AGENTS.md` files:
...
LoongClaw also keeps structured memory rows in SQLite (`memories` table):
...
- If MCP is not configured, unavailable, or returns invalid payloads, LoongClaw automatically falls back to built-in SQLite memory behavior.
...
LoongClaw now stores a channel-scoped identity for chats:
...
<p align="center">
  <img src="docs/assets/readme/skills-lifecycle.svg" alt="LoongClaw skill lifecycle diagram" width="92%" />
</p>

LoongClaw supports the [Anthropic Agent Skills](https://github.com/anthropics/skills) standard. Skills are modular packages that give the bot specialized capabilities for specific tasks.
...
LoongClaw supports manifest-based plugins for:
...
LoongClaw supports MCP servers configured in `<data_dir>/mcp.json` and optional fragments in `<data_dir>/mcp.d/*.json` with protocol negotiation and configurable transport.
...
[Peekaboo](https://github.com/steipete/Peekaboo) is a macOS desktop automation MCP server. LoongClaw can use it directly via `stdio` transport (no runtime code changes needed).
...
LoongClaw can also consume Windows desktop automation MCP servers via `stdio`.
...
Note: some Windows MCP projects expose only `sse` transport. LoongClaw runtime currently supports `stdio` and `streamable_http` transports only, so SSE-only servers need a protocol bridge before use.

## Plan & Execute

<p align="center">
  <img src="docs/assets/readme/plan-execute.svg" alt="LoongClaw plan and execute diagram" width="92%" />
</p>
...
<p align="center">
  <img src="docs/assets/readme/task-scheduler.svg" alt="LoongClaw scheduling flow diagram" width="92%" />
</p>
...
When `web_enabled: true`, LoongClaw serves a local Web UI (default `http://127.0.0.1:10961`).
...
- If no Web operator password exists, LoongClaw initializes a temporary default password `helloworld` and prompts you to change it after sign-in (you can skip temporarily)
- Password reset helpers:
  - `loongclaw web` (show usage)
  - `loongclaw web password <value>`
  - `loongclaw web password-generate`
  - `loongclaw web password-clear`
...
> **New:** LoongClaw now includes an interactive setup wizard (`loongclaw setup`) and will auto-launch it on first `start` when required config is missing.
...
2. Send `/newbot`
3. Enter a display name for your bot (e.g. `My LoongClaw`)
4. Enter a username (must end in `bot`, e.g. `my_loongclaw_bot`)
...
- Safe `loongclaw.config.yaml` save with automatic backup in `loongclaw.config.backups/` (keeps latest 50)
...
For `openai-codex`, you can run `codex login` first and LoongClaw will read OAuth from `~/.codex/auth.json` (or `$CODEX_HOME/auth.json`). You can also provide `api_key` when using an OpenAI-compatible proxy endpoint. The default base URL is `https://chatgpt.com/backend-api`.

You can still configure manually with `loongclaw.config.yaml`:
...
# llm_base_url: "https://..."
data_dir: "~/.loongclaw"
working_dir: "~/.loongclaw/working_dir"
...
```sh
loongclaw gateway install --force
loongclaw gateway start
loongclaw gateway stop
loongclaw gateway restart
loongclaw gateway logs 200
loongclaw gateway uninstall
```
...
- Gateway service stdout/stderr files are `loongclaw-gateway.log` and `loongclaw-gateway.error.log`.
- Logs older than 30 days are deleted automatically.

## Configuration

All configuration is via `loongclaw.config.yaml`:
...
| `data_dir` | No | `~/.loongclaw` | Data root (`runtime` data in `data_dir/runtime`, skills in `data_dir/skills`) |
| `working_dir` | No | `~/.loongclaw/working_dir` | Default working directory for tool operations; relative paths in `bash/read_file/write_file/edit_file/glob/grep` resolve from here |
...
| `channels.irc.real_name` | No | `"LoongClaw"` | IRC real name (sent in USER command) |
...
- If `data_dir` / `skills_dir` / `working_dir` are already configured, LoongClaw keeps using those configured paths.
- If these fields are not configured, defaults are `data_dir=~/.loongclaw`, `skills_dir=<data_dir>/skills`, `working_dir=~/.loongclaw/working_dir`.
...
- Runtime-controlled fields like stream mode and tool payload may still be set by LoongClaw for the active request path.
...
```sh
loongclaw setup --enable-sandbox
loongclaw doctor sandbox
```
...
sandbox:
  ...
  # mount_allowlist_path: "~/.loongclaw/sandbox-mount-allowlist.txt"
```
...
- Optional hardening:
  - `~/.loongclaw/sandbox-mount-allowlist.txt` for sandbox mount roots.
  - `~/.loongclaw/sandbox-path-allowlist.txt` for file tool path roots.
...
```
crates/
    loongclaw-core/      # Shared error/types/text modules
    loongclaw-storage/   # SQLite DB + memory domain + usage reporting
    loongclaw-tools/     # Tool runtime primitives + sandbox + helper engines
    loongclaw-channels/  # Channel abstractions and routing boundary
    loongclaw-app/       # App-level support modules (logging, builtin skills, transcribe)
...
LoongClaw's core loop is channel-agnostic. A new platform integration should mainly be an adapter layer:
