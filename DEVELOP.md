...
git clone <repo-url>
cd loongclaw
cp loongclaw.config.example.yaml loongclaw.config.yaml
# Edit loongclaw.config.yaml with your credentials
cargo run -- start
```
...
crates/
    ...
    loongclaw-storage/   # SQLite + memory + usage reporting
    loongclaw-tools/     # Tool runtime primitives + sandbox
    loongclaw-channels/  # Channel abstraction boundary
    loongclaw-app/       # App support modules (logging/skills/transcribe)
...
| `Database` | `loongclaw_storage::db` | SQLite wrapper with `Mutex<Connection>` |
| `ToolRegistry` | `tools/mod.rs` | Holds all `Box<dyn Tool>`, dispatches by name |
| `Tool` trait | `loongclaw_tools::runtime` | `name()`, `definition()`, `execute()` |
...
- `control_chat_ids` in `loongclaw.config.yaml` defines privileged chats.
...
# Just loongclaw logs
RUST_LOG=loongclaw=debug cargo run -- start
...
| Task | How |
|------|-----|
| Change the model | Set `model` in `loongclaw.config.yaml` based on `llm_provider` |
| Increase context window | Set `max_history_messages: 100` in `loongclaw.config.yaml` (uses more tokens) |
| Increase tool iterations | Set `max_tool_iterations: 200` in `loongclaw.config.yaml` |
| Reset memory | Delete files under `loongclaw.data/runtime/groups/` |
| Reset all data | Delete the `loongclaw.data/` directory |
| Tune compaction threshold | Set `max_session_messages: 60` in `loongclaw.config.yaml` (higher = more context before compaction) |
| Keep more recent messages | Set `compact_keep_recent: 30` in `loongclaw.config.yaml` (more recent messages kept verbatim) |
| Reset a chat session | Send `/reset` in the chat, or: `sqlite3 loongclaw.data/runtime/loongclaw.db "DELETE FROM sessions WHERE chat_id=XXXX;"` |
| Cancel all scheduled tasks | `sqlite3 loongclaw.data/runtime/loongclaw.db "UPDATE scheduled_tasks SET status='cancelled' WHERE status='active';"` |
...
The release binary is fully self-contained -- no runtime dependencies, no database server, no config files beyond `loongclaw.config.yaml`.
