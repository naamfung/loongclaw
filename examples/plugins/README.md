...
```sh
mkdir -p ~/.loongclaw/plugins
```

2. Copy one or both example manifests:

```sh
cp examples/plugins/smoke-test.yaml ~/.loongclaw/plugins/
cp examples/plugins/context-test.yaml ~/.loongclaw/plugins/
```

If you use a custom plugin directory, set it in `loongclaw.config.yaml`:
