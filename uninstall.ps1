...
function Resolve-Targets([string]$installDir, [string]$binName) {
  ...
  $cmd = Get-Command loongclaw -ErrorAction SilentlyContinue
  if ($cmd -and -not [string]::IsNullOrWhiteSpace($cmd.Source)) {
    $targets.Add($cmd.Source)
  }

  $targets.Add((Join-Path $env:USERPROFILE '.local\bin\loongclaw.exe'))
...
Write-Info "Uninstalling loongclaw..."
...
if ($removed -eq 0) {
  Write-Info "loongclaw binary not found. Nothing to uninstall."
  exit 0
}

Write-Info ""
Write-Info "loongclaw has been removed."
Write-Info "Optional cleanup (not removed automatically):"
Write-Info "  Remove-Item -Recurse -Force $HOME\\.loongclaw\\runtime"
Write-Info "  Remove-Item -Force .\\loongclaw.config.yaml,.\\loongclaw.config.yml"
