# LoongClaw Testing Guide

This document describes how to test every feature of LoongClaw. It includes both automated tests (unit/integration) and manual black-box functional tests organized by user stories.
...
Since LoongClaw is a multi-platform bot with external dependencies (LLM APIs, Telegram/Discord/WhatsApp APIs, DuckDuckGo), many features require live interaction testing.

### Prerequisites

1. A working `loongclaw.config.yaml` file with valid credentials
...
|---|-----------|-------|----------|
| 1.1 | First launch without config | Run `loongclaw start` with no config.yaml present | Auto-launches setup wizard |
| 1.2 | Normal startup | Fill config.yaml, run `loongclaw start` | Logs: Database initialized / Memory manager initialized / Scheduler started |
...
| 1.5 | CLI help | `loongclaw help` | Full output: commands, features, config docs |
| 1.6 | CLI version | `loongclaw version` | Output: `loongclaw {VERSION}` |
| 1.7 | Unknown command | `loongclaw foobar` | "Unknown command: foobar" + help text |
| 1.8 | Setup wizard | `loongclaw setup` | TUI interactive guide, provider/model selection |
| 1.9 | LOONGCLAW_CONFIG env var | `LOONGCLAW_CONFIG=/tmp/test.yaml loongclaw start` | Loads config from specified path |
...
| # | User Story | Steps | Expected |
|---|-----------|-------|----------|
| 25.1 | Install service | `loongclaw gateway install` | macOS: LaunchAgent; Linux: systemd service |
| 25.2 | Start service | `loongclaw gateway start` | Service starts |
| 25.3 | Check status | `loongclaw gateway status` | Reports running state |
| 25.4 | View logs | `loongclaw gateway logs 50` | Shows last 50 log lines |
| 25.5 | Stop service | `loongclaw gateway stop` | Service stops |
| 25.6 | Uninstall service | `loongclaw gateway uninstall` | Service removed |
...
```sh
sqlite3 loongclaw.data/runtime/loongclaw.db
...
```sh
# Remove test files
rm -f /tmp/loongclaw_test.txt /tmp/todos.txt /tmp/test.txt /tmp/session_test.txt

# Cancel any remaining scheduled tasks via the bot, or:
sqlite3 loongclaw.data/runtime/loongclaw.db "UPDATE scheduled_tasks SET status='cancelled' WHERE status='active';"
```
