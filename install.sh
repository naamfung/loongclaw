...
REPO="${LOONGCLAW_REPO:-loongclaw/loongclaw}"
BIN_NAME="loongclaw"
...
detect_install_dir() {
  if [ -n "${LOONGCLAW_INSTALL_DIR:-}" ]; then
    echo "$LOONGCLAW_INSTALL_DIR"
    return
  fi
...
extract_asset_url() {
  # Match assets like:
  #   loongclaw-0.0.5-aarch64-apple-darwin.tar.gz
  #   loongclaw-v0.0.5-aarch64-unknown-linux-gnu.tar.gz
...
main() {
  ...
  if [ -z "$asset_url" ]; then
    ...
    err "  Homebrew (macOS): brew tap loongclaw/tap && brew install loongclaw"
