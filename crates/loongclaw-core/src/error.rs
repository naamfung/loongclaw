...
mod tests {
    ...
    fn test_error_display_messages() {
        let e = LoongClawError::LlmApi("bad request".into());
        assert_eq!(e.to_string(), "LLM API error: bad request");

        let e = LoongClawError::RateLimited;
        assert_eq!(e.to_string(), "Rate limited, retry after backoff");

        let e = LoongClawError::ToolExecution("tool failed".into());
        assert_eq!(e.to_string(), "Tool execution error: tool failed");

        let e = LoongClawError::Config("missing key".into());
        assert_eq!(e.to_string(), "Config error: missing key");

        let e = LoongClawError::MaxIterations(25);
    ...
    fn test_error_from_io() {
        ...
        let e: LoongClawError = io_err.into();
    ...
    fn test_error_from_json() {
        ...
        let e: LoongClawError = json_err.into();
