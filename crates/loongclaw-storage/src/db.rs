...
use loongclaw_core::error::LoongClawError;
...
pub async fn call_blocking<T, F>(db: std::sync::Arc<Database>, f: F) -> Result<T, LoongClawError>
where
    T: Send + 'static,
    F: FnOnce(&Database) -> Result<T, LoongClawError> + Send + 'static,
{
    tokio::task::spawn_blocking(move || f(db.as_ref()))
        .await
        .map_err(|e| LoongClawError::ToolExecution(format!("DB task join error: {e}")))?
}
...
fn table_has_column(conn: &Connection, table: &str, column: &str) -> Result<bool, LoongClawError> {
    // Validate table name to prevent SQL injection via PRAGMA
    if !table.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') {
        return Err(LoongClawError::Config(format!(
            "invalid table name: {}",
            table
        )));
    }
...
fn ensure_memory_schema(conn: &Connection) -> Result<(), LoongClawError> {
...
fn ensure_chat_identity_schema(conn: &Connection) -> Result<(), LoongClawError> {
...
fn ensure_sessions_schema(conn: &Connection) -> Result<(), LoongClawError> {
...
fn get_schema_version(conn: &Connection) -> Result<i64, LoongClawError> {
    conn.execute(
...
fn set_schema_version(conn: &Connection, version: i64) -> Result<(), LoongClawError> {
    conn.execute(
...
fn apply_schema_migrations(conn: &Connection) -> Result<(), LoongClawError> {
...
impl Database {
    ...
    pub fn new(data_dir: &str) -> Result<Self, LoongClawError> {
        let db_path = Path::new(data_dir).join("loongclaw.db");
    ...
    pub fn store_message(&self, msg: &StoredMessage) -> Result<(), LoongClawError> {
        let conn = self.lock_conn();
        conn.execute(
    ...
    pub fn store_message_if_new(&self, msg: &StoredMessage) -> Result<bool, LoongClawError> {
        let conn = self.lock_conn();
    ...
    pub fn message_exists(&self, chat_id: i64, message_id: &str) -> Result<bool, LoongClawError> {
        let conn = self.lock_conn();
        let exists = conn
            .query_row(
    ...
    pub fn get_recent_messages(
        ...
        chat_id: i64,
        limit: usize,
    ) -> Result<Vec<StoredMessage>, LoongClawError> {
        let conn = self.lock_conn();
    ...
    pub fn get_all_messages(&self, chat_id: i64) -> Result<Vec<StoredMessage>, LoongClawError> {
        let conn = self.lock_conn();
        let mut stmt = conn.prepare(
            "SELECT id, chat_id, sender_name, content, is_from_bot, timestamp
             FROM messages
    ...
    pub fn get_chats_by_type(
        ...
        chat_type: &str,
        limit: usize,
    ) -> Result<Vec<ChatSummary>, LoongClawError> {
        let conn = self.lock_conn();
        let mut stmt = conn.prepare(
            "SELECT
                c.chat_id,
    ...
    pub fn get_recent_chats(&self, limit: usize) -> Result<Vec<ChatSummary>, LoongClawError> {
        let conn = self.lock_conn();
        let mut stmt = conn.prepare(
            "SELECT
                c.chat_id,
    ...
    pub fn get_chat_type(&self, chat_id: i64) -> Result<Option<String>, LoongClawError> {
        let conn = self.lock_conn();
    ...
    pub fn get_chat_id_by_channel_and_title(
        ...
        channel: &str,
        chat_title: &str,
    ) -> Result<Option<i64>, LoongClawError> {
        let conn = self.lock_conn();
        let result = conn.query_row(
            "SELECT chat_id
             FROM chats
    ...
    pub fn get_chat_channel(&self, chat_id: i64) -> Result<Option<String>, LoongClawError> {
        let conn = self.lock_conn();
        let result = conn.query_row(
    ...
    pub fn get_chat_external_id(&self, chat_id: i64) -> Result<Option<String>, LoongClawError> {
        let conn = self.lock_conn();
        let result = conn.query_row(
    ...
    pub fn get_due_tasks(&self, now: &str) -> Result<Vec<ScheduledTask>, LoongClawError> {
        let conn = self.lock_conn();
        let mut stmt = conn.prepare(
    ...
    pub fn get_tasks_for_chat(&self, chat_id: i64) -> Result<Vec<ScheduledTask>, LoongClawError> {
        let conn = self.lock_conn();
        let mut stmt = conn.prepare(
            "SELECT id, chat_id, prompt, schedule_type, schedule_value, next_run, last_run, status, created_at
             FROM scheduled_tasks
    ...
    pub fn get_task_by_id(&self, task_id: i64) -> Result<Option<ScheduledTask>, LoongClawError> {
        let conn = self.lock_conn();
        let result = conn.query_row(
            "SELECT id, chat_id, prompt, schedule_type, schedule_value, next_run, last_run, status, created_at
             FROM scheduled_tasks
    ...
    pub fn update_task_status(&self, task_id: i64, status: &str) -> Result<bool, LoongClawError> {
        let conn = self.lock_conn();
    ...
    pub fn get_task_run_logs(
        ...
        task_id: i64,
        limit: usize,
    ) -> Result<Vec<TaskRunLog>, LoongClawError> {
        let conn = self.lock_conn();
        let mut stmt = conn.prepare(
    ...
    pub fn list_scheduled_task_dlq(
        ...
        limit: usize,
    ) -> Result<Vec<ScheduledTaskDlqEntry>, LoongClawError> {
        let conn = self.lock_conn();
    ...
    #[allow(dead_code)]
    pub fn delete_task(&self, task_id: i64) -> Result<bool, LoongClawError> {
        let conn = self.lock_conn();
        let rows = conn.execute(
    ...
    pub fn save_session(&self, chat_id: i64, messages_json: &str) -> Result<(), LoongClawError> {
    ...
    pub fn load_session(&self, chat_id: i64) -> Result<Option<(String, String)>, LoongClawError> {
        let conn = self.lock_conn();
        let result = conn.query_row(
    ...
    pub fn load_session_meta(
        &self,
        chat_id: i64,
    ) -> Result<Option<SessionMetaRow>, LoongClawError> {
        let conn = self.lock_conn();
        let result = conn.query_row(
    ...
    pub fn list_session_meta(&self, limit: usize) -> Result<Vec<SessionTreeRow>, LoongClawError> {
        let conn = self.lock_conn();
        let mut stmt = conn.prepare(
    ...
    pub fn delete_session(&self, chat_id: i64) -> Result<bool, LoongClawError> {
        let conn = self.lock_conn();
    ...
    pub fn clear_chat_context(&self, chat_id: i64) -> Result<bool, LoongClawError> {
        let conn = self.lock_conn();
    ...
    pub fn delete_chat_data(&self, chat_id: i64) -> Result<bool, LoongClawError> {
        let conn = self.lock_conn();
        let tx = conn.unchecked_transaction()?;
        let mut affected = 0usize;
    ...
    pub fn upsert_auth_password_hash(&self, password_hash: &str) -> Result<(), LoongClawError> {
        let conn = self.lock_conn();
        let now = chrono::Utc::now().to_rfc3339();
        conn.execute(
    ...
    pub fn get_auth_password_hash(&self) -> Result<Option<String>, LoongClawError> {
        let conn = self.lock_conn();
        let value = conn
            .query_row(
    ...
    pub fn clear_auth_password_hash(&self) -> Result<bool, LoongClawError> {
        let conn = self.lock_conn();
    ...
    pub fn validate_auth_session(&self, session_id: &str) -> Result<bool, LoongClawError> {
        let conn = self.lock_conn();
        let now = chrono::Utc::now().to_rfc3339();
        let valid = conn
            .query_row(
    ...
    pub fn revoke_auth_session(&self, session_id: &str) -> Result<bool, LoongClawError> {
        let conn = self.lock_conn();
        let now = chrono::Utc::now().to_rfc3339();
        let rows = conn.execute(
    ...
    pub fn revoke_all_auth_sessions(&self) -> Result<usize, LoongClawError> {
        let conn = self.lock_conn();
        let now = chrono::Utc::now().to_rfc3339();
        let rows = conn.execute(
            "UPDATE auth_sessions
    ...
    pub fn list_api_keys(&self) -> Result<Vec<AuthApiKeyRecord>, LoongClawError> {
        let conn = self.lock_conn();
        let mut stmt = conn.prepare(
    ...
    pub fn rotate_api_key_revoke_old(&self, old_key_id: i64) -> Result<bool, LoongClawError> {
        self.revoke_api_key(old_key_id)
    }

    pub fn revoke_api_key(&self, key_id: i64) -> Result<bool, LoongClawError> {
        let conn = self.lock_conn();
        let now = chrono::Utc::now().to_rfc3339();
        let rows = conn.execute(
            "UPDATE api_keys
    ...
    pub fn validate_api_key_hash(
        &self,
        key_hash: &str,
    ) -> Result<Option<(i64, Vec<String>)>, LoongClawError> {
        let conn = self.lock_conn();
        let row = conn
            .query_row(
    ...
    pub fn list_audit_logs(
        ...
        kind: Option<&str>,
        limit: usize,
    ) -> Result<Vec<AuditLogRecord>, LoongClawError> {
        let conn = self.lock_conn();
    ...
    pub fn get_metrics_history(
        ...
        since_ts_ms: i64,
        limit: usize,
    ) -> Result<Vec<MetricsHistoryPoint>, LoongClawError> {
        let conn = self.lock_conn();
        let mut stmt = conn.prepare(
            "SELECT
                timestamp_ms, llm_completions, llm_input_tokens, llm_output_tokens,
    ...
    pub fn get_reflector_cursor(&self, chat_id: i64) -> Result<Option<String>, LoongClawError> {
        let conn = self.lock_conn();
        let result = conn.query_row(
    ...
    pub fn get_llm_usage_summary(
        &self,
        chat_id: Option<i64>,
    ) -> Result<LlmUsageSummary, LoongClawError> {
    ...
    pub fn get_llm_usage_by_model(
        ...
        since: Option<&str>,
        limit: Option<usize>,
    ) -> Result<Vec<LlmModelUsageSummary>, LoongClawError> {
        let conn = self.lock_conn();
    ...
    pub fn get_memories_for_context(
        ...
        chat_id: i64,
        limit: usize,
    ) -> Result<Vec<Memory>, LoongClawError> {
        let conn = self.lock_conn();
        let mut stmt = conn.prepare(
    ...
    pub fn get_active_chat_ids_since(&self, since: &str) -> Result<Vec<i64>, LoongClawError> {
        let conn = self.lock_conn();
        let mut stmt = conn.prepare(
    ...
    pub fn delete_memory(&self, id: i64) -> Result<bool, LoongClawError> {
        let conn = self.lock_conn();
    ...
    #[cfg(feature = "sqlite-vec")]
    pub fn prepare_vector_index(&self, dimension: usize) -> Result<(), LoongClawError> {
        let conn = self.lock_conn();
    ...
    pub fn get_all_active_memories(&self) -> Result<Vec<(i64, String)>, LoongClawError> {
        let conn = self.lock_conn();
        let mut stmt =
    ...
    pub fn knn_memories(
        ...
        query_vec: &[f32],
        k: usize,
    ) -> Result<Vec<(i64, f32)>, LoongClawError> {
        let conn = self.lock_conn();
    ...
    pub fn get_memory_by_id(&self, id: i64) -> Result<Option<Memory>, LoongClawError> {
        let conn = self.lock_conn();
        let result = conn.query_row(
            "SELECT id, chat_id, content, category, created_at, updated_at, embedding_model,
                    confidence, source, last_seen_at, is_archived, archived_at
    ...
    pub fn archive_memory(&self, id: i64) -> Result<bool, LoongClawError> {
        let conn = self.lock_conn();
        let now = chrono::Utc::now().to_rfc3339();
        let rows = conn.execute(
            "UPDATE memories
    ...
    pub fn archive_stale_memories(&self, stale_days: i64) -> Result<usize, LoongClawError> {
        let conn = self.lock_conn();
    ...
    pub fn get_memory_observability_summary(
        &self,
        chat_id: Option<i64>,
    ) -> Result<MemoryObservabilitySummary, LoongClawError> {
        let conn = self.lock_conn();
    ...
    pub fn get_memory_reflector_runs(
        ...
        limit: usize,
        offset: usize,
    ) -> Result<Vec<MemoryReflectorRun>, LoongClawError> {
        let conn = self.lock_conn();
        let mut query = String::from(
    ...
    pub fn get_memory_injection_logs(
        ...
        limit: usize,
        offset: usize,
    ) -> Result<Vec<MemoryInjectionLog>, LoongClawError> {
        let conn = self.lock_conn();
        let mut query = String::from(
...
mod tests {
    ...
    fn test_db() -> (Database, std::path::PathBuf) {
        let dir = std::env::temp_dir().join(format!("loongclaw_test_{}", uuid::Uuid::new_v4()));
    ...
    fn test_legacy_schema_is_upgraded_to_current_version() {
        let dir =
            std::env::temp_dir().join(format!("loongclaw_legacy_upgrade_{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let db_path = dir.join("loongclaw.db");
    ...
    fn test_migration_backfills_chat_identity_columns() {
        let dir = std::env::temp_dir().join(format!(
            "loongclaw_migration_chat_identity_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&dir).unwrap();
        let db_path = dir.join("loongclaw.db");
    ...
    fn test_migration_backfills_memory_identity_columns() {
        let dir = std::env::temp_dir().join(format!(
            "loongclaw_migration_memory_identity_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&dir).unwrap();
        let db_path = dir.join("loongclaw.db");
