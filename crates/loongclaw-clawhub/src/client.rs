...
impl ClawHubClient {
    ...
    pub async fn search(
        ...
        let resp = req
            .send()
            .await
            .map_err(|e| LoongClawError::Config(format!("ClawHub request failed: {}", e)))?;
        let search_response: ApiSearchResponse = resp.json().await.map_err(|e| {
            LoongClawError::Config(format!("Failed to parse search results: {}", e))
        })?;
    ...
    pub async fn get_skill(&self, slug: &str) -> Result<SkillMeta, LoongClawError> {
        ...
        let get_response: GetSkillResponse = resp.json().await.map_err(|e| {
            LoongClawError::Config(format!("Failed to parse skill metadata: {}", e))
        })?;
    ...
    pub async fn download_skill(
        ...
        let mut last_error: Option<LoongClawError> = None;
        for url in candidate_urls {
            ...
            let resp = match req.send().await {
                Ok(resp) => resp,
                Err(e) => {
                    last_error = Some(LoongClawError::Config(format!(
            ...
            let bytes = resp.bytes().await.map_err(|e| {
                LoongClawError::Config(format!("Failed to read download from {}: {}", url, e))
            })?;
            return Ok(bytes.to_vec());
        }

        Err(last_error.unwrap_or_else(|| {
            LoongClawError::Config("ClawHub download failed: no usable endpoint".into())
        }))
    }

    /// List versions for a skill
    pub async fn get_versions(&self, slug: &str) -> Result<Vec<SkillVersion>, LoongClawError> {
        ...
        let versions: Vec<SkillVersion> = resp
            .json()
            .await
            .map_err(|e| LoongClawError::Config(format!("Failed to parse versions: {}", e)))?;
        Ok(versions)
    }
}
