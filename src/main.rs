...
use loongclaw::config::Config;
use loongclaw::error::LoongClawError;
use loongclaw::{
...
const LONG_ABOUT: &str = concat!(
    ...
    "\x1b[22m\n",
    "\x1b[1mWebsite:\x1b[22m https://loongclaw.ai\n",
    "\x1b[1mGitHub:\x1b[22m https://github.com/loongclaw/loongclaw\n",
...
Usage:
  loongclaw web [password <value> | password-generate | password-clear]
...
fn handle_web_cli(action: Option<WebAction>) -> anyhow::Result<()> {
    ...
    if matches!(action, Some(WebAction::PasswordClear)) {
        ...
        println!(
            "State is now uninitialized. On next `loongclaw start`, default password bootstrap policy will apply."
        );
        return Ok(());
    }
...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Some(MainCommand::Setup(setup_args)) => {
            if setup_args.enable_sandbox {
                ...
                if !setup_args.yes && !setup_args.quiet {
                    println!(
                        "Tip: run `loongclaw doctor sandbox` to verify docker runtime and image readiness."
                    );
                }
            } else {
                let saved = setup::run_setup_wizard()?;
                if saved {
                    println!("Setup saved to loongclaw.config.yaml");
                } else {
        ...
        Some(MainCommand::Skill { args }) => {
            let config = Config::load()?;
            loongclaw::clawhub::cli::handle_skill_cli(&args, &config).await?;
            return Ok(());
        }
    ...
    if std::env::var("LOONGCLAW_GATEWAY").is_ok() {
...
mod tests {
    ...
    fn unique_temp_dir() -> std::path::PathBuf {
        let dir =
            std::env::temp_dir().join(format!("loongclaw-main-test-{}", uuid::Uuid::new_v4()));
