...
use loongclaw_core::llm_types::{
    ContentBlock, ImageSource, Message, MessageContent, ResponseContentBlock,
};
use loongclaw_core::text::floor_char_boundary;
use loongclaw_storage::db::{call_blocking, Database, StoredMessage};
use loongclaw_storage::memory_quality;
...
fn with_high_risk_approval_marker(input: &Value) -> Value {
    ...
    if let Some(obj) = approved_input.as_object_mut() {
        obj.insert(
            "__loongclaw_high_risk_approved".to_string(),
            Value::Bool(true),
        );
        return approved_input;
    }
    serde_json::json!({
        "__loongclaw_high_risk_approved": true,
        "__loongclaw_original_input": input,
    })
}
...
pub(crate) async fn build_db_memory_context(
    ...
    let mut ordered: Vec<&loongclaw_storage::db::Memory> = Vec::new();
    #[cfg(feature = "sqlite-vec")]
            ...
            if memory_backend.prefers_mcp() {
                ...
                if let Ok(query_vec) = provider.embed(query).await {
                    ...
                    if let Ok(knn_rows) = knn_result {
                        let by_id: std::collections::HashMap<i64, &loongclaw_storage::db::Memory> =
    ...
    if ordered.is_empty() {
        ...
        let mut scored: Vec<(usize, usize, &loongclaw_storage::db::Memory)> = memories
            .iter()
            .enumerate()
            .map(|(idx, m)| {
                (
...
- Install skills from repos (`sync_skills`, `clawhub_install`, `clawhub_search`) — use these instead of manually writing SKILL.md files. Skills go in ~/.loongclaw/skills/ (or configured skills dir).
...
mod tests {
    ...
    use loongclaw_storage::db::{Database, StoredMessage};
    use serde_json::json;
    ...
    impl LlmProvider for DummyLlm {
        ...
        ) -> Result<MessagesResponse, LoongClawError> {
            Ok(MessagesResponse {
    ...
    impl LlmProvider for EmptyVisibleThenNormalLlm {
        async fn send_message(
            ...
            let saw_guard = messages.iter().any(|m| match &m.content {
                loongclaw_core::llm_types::MessageContent::Text(t) => {
    ...
    impl LlmProvider for ApprovalLoopUntilSuccessfulToolLlm {
        async fn send_message(
            ...
            for msg in messages.iter().rev() {
                ...
                if let loongclaw_core::llm_types::MessageContent::Blocks(blocks) = &msg.content {
                    for block in blocks {
                        if let loongclaw_core::llm_types::ContentBlock::ToolResult {
                            content,
                            is_error,
                            ..
                        } = block
