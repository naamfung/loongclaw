...
use loongclaw_tools::sandbox::{SandboxExecOptions, SandboxRouter};
...
impl Tool for BashTool {
        ...
        ToolDefinition {
            ...
            input_schema: schema_object(
                json!({
                    ...
                    "timeout_secs": {
                        "type": "integer",
                        "description": "Timeout in seconds (defaults to configured tool timeout budget). For operations that may take time (like git clone, downloads, or builds), set a longer timeout (e.g., 500 seconds for git clone of large repositories)."
                    }
                }),
                &["command"],
            ),
    ...
    async fn execute(&self, input: serde_json::Value) -> ToolResult {
        ...
        let result = if let Some(router) = &self.sandbox_router {
            ...
            loongclaw_tools::sandbox::exec_host_command(command, &exec_opts).await
        };
...
mod tests {
    ...
    async fn test_bash_uses_working_dir() {
        let root = std::env::temp_dir().join(format!("loongclaw_bash_{}", uuid::Uuid::new_v4()));
