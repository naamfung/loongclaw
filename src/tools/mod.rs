...
use loongclaw_storage::db::Database;
pub use loongclaw_tools::runtime::{
...
use loongclaw_tools::runtime::{inject_auth_context, require_high_risk_approval};
use loongclaw_tools::sandbox::{SandboxMode, SandboxRouter};
...
impl ToolRegistry {
    pub fn new(
        ...
        let mut tools: Vec<Box<dyn Tool>> = vec![
            ...
            Box::new(
                browser::BrowserTool::new(&config.data_dir, config.browser_executable_path.clone())
    ...
    pub async fn execute_with_auth(
        ...
        // Check if web host is local (127.x.x.x, 192.168.x.x, localhost, ::1)
        let is_local_web_host = auth.caller_channel == "web" && {
            let host = &self.config.web_host;
            let h = host.trim().to_ascii_lowercase();
            h == "127.0.0.1" || h == "localhost" || h == "::1" || h.starts_with("192.168.")
        };
        if !is_local_web_host {
...
mod tests {
    ...
    async fn test_high_risk_tool_requires_explicit_approval_on_web() {
        ...
        let approved = registry
            .execute_with_auth(
                "bash",
                json!({"__loongclaw_high_risk_approved": true}),
                &auth,
            )
            .await;
    ...
    async fn test_dynamic_plugin_tool_executes_without_restart() {
        let root = std::env::temp_dir().join(format!("loongclaw_plugin_{}", uuid::Uuid::new_v4()));
