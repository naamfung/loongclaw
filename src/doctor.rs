...
fn check_config(report: &mut DoctorReport) {
    match Config::resolve_config_path() {
        ...
        Ok(None) => report.push(
            ...
            "Config file",
            CheckStatus::Warn,
            "loongclaw.config.yaml not found".to_string(),
            Some("Run `loongclaw setup` to create configuration.".to_string()),
        ),
        Err(err) => report.push(
            "config.file",
            "Config file",
            CheckStatus::Fail,
            err.to_string(),
            Some("Fix LOONGCLAW_CONFIG or create a valid config file.".to_string()),
        ),
    }
}
...
fn check_path(report: &mut DoctorReport) {
    ...
    } else if command_exists("loongclaw") {
        report.push(
            "path.loongclaw",
            "loongclaw in PATH",
            CheckStatus::Pass,
            "loongclaw is discoverable in PATH".to_string(),
            None,
        );
    } else {
        report.push(
            ...
            "loongclaw in PATH",
            CheckStatus::Warn,
            "loongclaw is not discoverable in PATH".to_string(),
            Some("Add the loongclaw binary directory to PATH.".to_string()),
        );
    }
}
...
fn check_browser_dependency(report: &mut DoctorReport) {
    ...
    let browser_cmds = if cfg!(target_os = "windows") {
        ["C:/Program Files/Google/Chrome/chrome.exe", "chrome.exe", "", ""]
    } else if cfg!(target_os = "macos") {
        ["/Applications/Google Chrome.app/Contents/MacOS/Google Chrome", "chromium-browser", "google-chrome", ""]
    } else {
        ["/usr/bin/chromium-browser", "/usr/bin/google-chrome", "chromium-browser", "google-chrome"]
    };
    ...
    report.push(
        ...
        if found {
            format!("Browser executable found: {found_path}")
        } else {
            "No browser executable found".to_string()
        },
        if found {
            None
        } else {
            Some("Install a browser (Chromium or Google Chrome) and ensure it's in PATH or configure browser_executable_path in config.".to_string())
        },
    );
}
...
fn check_mcp_dependencies(report: &mut DoctorReport) {
    let data_root = match Config::load() {
        ...
        Err(_) => PathBuf::from("./loongclaw.data"),
    };
...
fn check_sandbox_config(report: &mut DoctorReport) {
    let config = match Config::load() {
        ...
        Err(err) => {
            report.push(
                ...
                Some("Run `loongclaw setup` first.".to_string()),
            );
            return;
        }
    };
    ...
    report.push(
        ...
        if matches!(config.sandbox.mode, SandboxMode::Off) {
            Some("Enable quickly: `loongclaw setup --enable-sandbox`.".to_string())
        } else {
            None
        },
    );
...
fn default_mount_allowlist_path() -> Option<PathBuf> {
    user_home_dir().map(|h| h.join(".loongclaw/sandbox-mount-allowlist.txt"))
}

fn print_report(report: &DoctorReport) {
    println!("LoongClaw Doctor");
    println!(
...
mod tests {
    ...
    fn test_build_report_has_web_fetch_validation_checks() {
        let _guard = env_lock();
        let path = std::env::temp_dir().join(format!(
            "loongclaw_doctor_webfetch_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_build_report_warns_when_feed_sync_enabled_without_sources() {
        let _guard = env_lock();
        let path = std::env::temp_dir().join(format!(
            "loongclaw_doctor_feed_sync_warn_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
        let mut cfg = Config::test_defaults();
    ...
    fn test_build_report_fails_invalid_feed_source_url() {
        let _guard = env_lock();
        let path = std::env::temp_dir().join(format!(
            "loongclaw_doctor_feed_sync_invalid_url_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
        let mut cfg = Config::test_defaults();
        cfg.web_fetch_url_validation.feed_sync.enabled = true;
        ...
        cfg.web_fetch_url_validation.feed_sync.sources =
            vec![loongclaw_tools::web_fetch::WebFetchFeedSource {
                enabled: true,
                mode: loongclaw_tools::web_fetch::WebFetchFeedMode::Denylist,
                url: "notaurl".to_string(),
                format: loongclaw_tools::web_fetch::WebFetchFeedFormat::Lines,
