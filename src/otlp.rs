...
impl OtlpExporter {
    pub fn from_config(config: &Config) -> Option<Arc<Self>> {
        ...
        let service_name = map
            ...
            .filter(|s| !s.is_empty())
            .unwrap_or_else(|| "loongclaw".to_string());
...
mod tests {
    ...
    fn test_build_otlp_payload() {
        ...
        assert!(metrics.iter().any(|m| m.name == "loongclaw_http_requests"));
        assert!(metrics
            .iter()
            .any(|m| m.name == "loongclaw_mcp_rate_limited_rejections"));
        assert!(metrics
            .iter()
            .any(|m| m.name == "loongclaw_mcp_bulkhead_rejections"));
        assert!(metrics
            .iter()
            .any(|m| m.name == "loongclaw_mcp_circuit_open_rejections"));
    }
