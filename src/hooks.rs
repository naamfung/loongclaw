...
impl HookManager {
    ...
    pub fn for_tests() -> Self {
        let tmp = std::env::temp_dir().join(format!("loongclaw-hooks-{}", uuid::Uuid::new_v4()));
...
async fn run_hook_command(
    ...
    command
        ...
        .env("LOONGCLAW_HOOK_EVENT", event.as_str())
        .env("LOONGCLAW_HOOK_NAME", &hook.name);
