    ...
    async fn send_message_stream_single_pass(
        ...
        if !status.is_success() {
            ...
            if let Ok(api_err) = serde_json::from_str::<AnthropicApiError>(&body) {
                return Err(LoongClawError::LlmApi(format!(
                    "{}: {}",
                    api_err.error.error_type, api_err.error.message
                )));
            }
            return Err(LoongClawError::LlmApi(format!("HTTP {status}: {body}")));
        }
        ...
        loop {
            ...
            if status.is_success() {
                ...
                let parsed: MessagesResponse = serde_json::from_str(&body).map_err(|e| {
                    LoongClawError::LlmApi(format!("Failed to parse response: {e}\nBody: {body}"))
                })?;
                return Ok(parsed);
            }
    ...
    async fn send_message_with_model(
        ...
        loop {
            ...
            if let Ok(err) = serde_json::from_str::<OaiErrorResponse>(&text) {
                return Err(LoongClawError::LlmApi(err.error.message));
            }
            return Err(LoongClawError::LlmApi(format!("HTTP {status}: {text}")));
        }
    }
...
fn parse_openai_codex_response_payload(text: &str) -> Result<OaiResponsesResponse, LoongClawError> {
    ...
    Err(LoongClawError::LlmApi(format!(
...
mod tests {
    ...
    fn test_resolve_openai_compat_base_defaults_openai_codex() {
        ...
        let temp = std::env::temp_dir().join(format!(
            "loongclaw-llm-codex-base-default-{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap_or_default()
                .as_nanos()
