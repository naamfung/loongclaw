...
use loongclaw_channels::channel::ConversationKind;
use loongclaw_channels::channel::{
    deliver_and_store_bot_message, get_chat_routing, session_source_for_chat,
};
use loongclaw_channels::channel_adapter::{ChannelAdapter, ChannelRegistry};
use loongclaw_core::llm_types::Message;
use loongclaw_storage::db::{call_blocking, ChatSummary, MetricsHistoryPoint, StoredMessage};
use loongclaw_storage::usage::build_usage_report;
...
fn config_path_for_save() -> Result<PathBuf, (StatusCode, String)> {
    match Config::resolve_config_path() {
        ...
        Ok(None) => Ok(PathBuf::from("./loongclaw.config.yaml")),
...
pub async fn start_web_server(state: Arc<AppState>) {
    ...
    // Generate bootstrap token if no password is set, regardless of web_auth_token
    if !has_password {
        // Generate bootstrap token for first-time setup
        bootstrap_token = Some(uuid::Uuid::new_v4().to_string());
        warn!("Web UI bootstrap token: {}", bootstrap_token.as_ref().unwrap());
    }
...
mod tests {
    ...
    use crate::{error::LoongClawError, llm_types::ResponseContentBlock};
    use axum::body::Body;
    use axum::http::{Request, StatusCode};
    use loongclaw_channels::channel_adapter::ChannelRegistry;
    use loongclaw_storage::db::call_blocking;
    use serde_json::json;
    ...
    impl LlmProvider for DummyLlm {
        async fn send_message(
            &self,
            _system: &str,
            _messages: Vec<loongclaw_core::llm_types::Message>,
            _tools: Option<Vec<loongclaw_core::llm_types::ToolDefinition>>,
        ) -> Result<
            loongclaw_core::llm_types::MessagesResponse,
            loongclaw_core::error::LoongClawError,
        > {
            Ok(loongclaw_core::llm_types::MessagesResponse {
                content: vec![loongclaw_core::llm_types::ResponseContentBlock::Text {
    ...
    impl LlmProvider for SlowLlm {
        async fn send_message(
            ...
            _messages: Vec<loongclaw_core::llm_types::Message>,
            _tools: Option<Vec<loongclaw_core::llm_types::ToolDefinition>>,
        ) -> Result<loongclaw_core::llm_types::MessagesResponse, LoongClawError> {
    ...
    impl LlmProvider for ToolFlowLlm {
        async fn send_message(
            ...
            if n == 0 {
                return Ok(loongclaw_core::llm_types::MessagesResponse {
    ...
    fn test_state_with_config(llm: Box<dyn LlmProvider>, mut cfg: Config) -> Arc<AppState> {
        let dir = std::env::temp_dir().join(format!("loongclaw_webtest_{}", uuid::Uuid::new_v4()));
    ...
    async fn test_web_send_plugin_slash_command() {
        let mut cfg = test_config_template();
        let plugin_dir =
            std::env::temp_dir().join(format!("loongclaw_web_plugin_{}", uuid::Uuid::new_v4()));
