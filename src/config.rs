...
pub use loongclaw_tools::sandbox::{SandboxBackend, SandboxConfig, SandboxMode, SecurityProfile};
pub use loongclaw_tools::types::WorkingDirIsolation;
use loongclaw_tools::web_content_validation::WebContentValidationConfig;
use loongclaw_tools::web_fetch::WebFetchUrlValidationConfig;
...
fn default_data_root() -> PathBuf {
    home_dir()
        .map(|h| h.join(".loongclaw"))
        .unwrap_or_else(|| PathBuf::from(".loongclaw"))
}
...
impl Config {
    ...
    /// Skills directory. Priority: LOONGCLAW_SKILLS_DIR env var > skills_dir config > <data_dir>/skills
    pub fn skills_data_dir(&self) -> String {
        // 1. Check env var first
        if let Ok(explicit_dir) = std::env::var("LOONGCLAW_SKILLS_DIR") {
    ...
    pub fn resolve_config_path() -> Result<Option<PathBuf>, LoongClawError> {
        // 1. Check LOONGCLAW_CONFIG env var for custom path
        if let Ok(custom) = std::env::var("LOONGCLAW_CONFIG") {
            if std::path::Path::new(&custom).exists() {
                return Ok(Some(PathBuf::from(custom)));
            }
            return Err(LoongClawError::Config(format!(
                "LOONGCLAW_CONFIG points to non-existent file: {custom}"
            )));
        }

        if std::path::Path::new("./loongclaw.config.yaml").exists() {
            return Ok(Some(PathBuf::from("./loongclaw.config.yaml")));
        }
        if std::path::Path::new("./loongclaw.config.yml").exists() {
            return Ok(Some(PathBuf::from("./loongclaw.config.yml")));
        }
        Ok(None)
    }
    ...
    pub fn load() -> Result<Self, LoongClawError> {
        let yaml_path = Self::resolve_config_path()?;

        if let Some(path) = yaml_path {
            let path_str = path.to_string_lossy().to_string();
            let content = std::fs::read_to_string(&path)
                .map_err(|e| LoongClawError::Config(format!("Failed to read {path_str}: {e}")))?;
            let mut config: Config = serde_yaml::from_str(&content)
                .map_err(|e| LoongClawError::Config(format!("Failed to parse {path_str}: {e}")))?;
            config.post_deserialize()?;
            return Ok(config);
        }

        // No config file found at all
        Err(LoongClawError::Config(
            "No loongclaw.config.yaml found. Run `loongclaw setup` to create one.".into(),
        ))
    }

    /// Apply post-deserialization normalization and validation.
    pub(crate) fn post_deserialize(&mut self) -> Result<(), LoongClawError> {
        ...
        self.timezone
            .parse::<chrono_tz::Tz>()
            .map_err(|_| LoongClawError::Config(format!("Invalid timezone: {}", self.timezone)))?;
        ...
        if self.api_key.is_empty() && !provider_allows_empty_api_key(&self.llm_provider) {
            return Err(LoongClawError::Config("api_key is required".into()));
        }
        if is_openai_codex_provider(&self.llm_provider) {
            if !self.api_key.trim().is_empty() {
                return Err(LoongClawError::Config(
                    "openai-codex ignores loongclaw.config.yaml api_key. Configure ~/.codex/auth.json or run `codex login` instead.".into(),
                ));
            }
            if self
                .llm_base_url
            ...
            {
                return Err(LoongClawError::Config(
                    "openai-codex ignores loongclaw.config.yaml llm_base_url. Configure ~/.codex/config.toml instead.".into(),
                ));
            }
    ...
    #[allow(dead_code)]
    pub fn save_yaml(&self, path: &str) -> Result<(), LoongClawError> {
        let content = serde_yaml::to_string(self)
            .map_err(|e| LoongClawError::Config(format!("Failed to serialize config: {e}")))?;
...
"#;
    ...
    fn test_config_default_values() {
        ...
        assert!(config.data_dir.ends_with(".loongclaw"));
        assert!(std::path::PathBuf::from(&config.working_dir)
            .ends_with(std::path::Path::new(".loongclaw").join("working_dir")));
    ...
    #[test]
    fn test_default_data_dir_uses_loongclaw_home() {
        let yaml = "telegram_bot_token: tok\nbot_username: bot\napi_key: key\n";
        let config: Config = serde_yaml::from_str(yaml).unwrap();
        assert!(config.data_dir.ends_with(".loongclaw"));
    }
    ...
    fn test_runtime_and_skills_dirs_from_root_data_dir() {
        let mut config = test_config();
        config.data_dir = "./loongclaw.data".into();

        let runtime = std::path::PathBuf::from(config.runtime_data_dir());
        let skills = std::path::PathBuf::from(config.skills_data_dir());

        assert!(runtime.ends_with(std::path::Path::new("loongclaw.data").join("runtime")));
        assert!(skills.ends_with(std::path::Path::new("loongclaw.data").join("skills")));
    }

    #[test]
    fn test_runtime_and_skills_dirs_from_runtime_data_dir() {
        let mut config = test_config();
        config.data_dir = "./loongclaw.data/runtime".into();

        let runtime = std::path::PathBuf::from(config.runtime_data_dir());
        let skills = std::path::PathBuf::from(config.skills_data_dir());

        assert!(runtime.ends_with(
            std::path::Path::new("loongclaw.data")
                .join("runtime")
                .join("runtime")
        ));
    ...
    fn test_skills_dir_uses_config_override() {
        let mut config = test_config();
        config.skills_dir = Some("./loongclaw.data/skills".to_string());
        let skills = std::path::PathBuf::from(config.skills_data_dir());
        assert!(skills.ends_with(std::path::Path::new("loongclaw.data").join("skills")));
    }
...
channels:
  matrix:
    ...
    bot_user_id: "@loongclaw:example.com"
"##;
        let mut config: Config = serde_yaml::from_str(yaml).unwrap();
        config.post_deserialize().unwrap();
    ...
    fn test_post_deserialize_openai_codex_rejects_plain_api_key_without_oauth() {
        ...
        let auth_dir = std::env::temp_dir().join(format!(
            "loongclaw-codex-auth-plain-key-{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap_or_default()
                .as_nanos()
        ...
        assert!(msg.contains("ignores loongclaw.config.yaml api_key"));
    }
...
"#;
    ...
    fn test_config_save_yaml() {
        ...
        let path = dir.join("loongclaw_test_config.yaml");
