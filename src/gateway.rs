...
const LINUX_SERVICE_NAME: &str = "loongclaw-gateway.service";
const MAC_LABEL: &str = "ai.loongclaw.gateway";
const LOG_STDOUT_FILE: &str = "loongclaw-gateway.log";
const LOG_STDERR_FILE: &str = "loongclaw-gateway.error.log";
...
fn resolve_config_path(cwd: &Path) -> Option<PathBuf> {
    if let Ok(from_env) = std::env::var("LOONGCLAW_CONFIG") {
    ...
    for candidate in ["loongclaw.config.yaml", "loongclaw.config.yml"] {
...
fn build_service_env(config_path: Option<&PathBuf>) -> BTreeMap<String, String> {
    ...
    env.insert("LOONGCLAW_GATEWAY".to_string(), "1".to_string());

    if let Some(path) = config_path {
        env.insert(
            "LOONGCLAW_CONFIG".to_string(),
...
fn render_linux_unit(ctx: &ServiceContext) -> Result<String> {
    ...
    unit.push_str("[Unit]\n");
    unit.push_str("Description=LoongClaw Gateway Service\n");
...
fn audit_linux_unit(ctx: &ServiceContext, runtime: &LinuxRuntimeStatus) -> Vec<String> {
    ...
    if !content.contains(&expected_exec) {
        issues.push("Service ExecStart does not match current loongclaw binary".to_string());
    }

    if let Some(config_path) = &ctx.config_path {
        let config_kv = format!("LOONGCLAW_CONFIG={}", config_path.display());
        if !content.contains(&config_kv) {
            issues.push("Service LOONGCLAW_CONFIG differs from current config path".to_string());
        }
    }
...
fn start_macos() -> Result<()> {
    ...
    if !plist_path.exists() {
        return Err(anyhow!(
            "Service not installed. Run: loongclaw gateway install"
        ));
    }
...
mod tests {
    ...
    fn test_ctx() -> ServiceContext {
        ...
        service_env.insert("LOONGCLAW_GATEWAY".to_string(), "1".to_string());
        service_env.insert(
            "LOONGCLAW_CONFIG".to_string(),
            "/tmp/loongclaw/loongclaw.config.yaml".to_string(),
        );

        ServiceContext {
            exe_path: PathBuf::from("/usr/local/bin/loongclaw"),
            working_dir: PathBuf::from("/tmp/loongclaw"),
            config_path: Some(PathBuf::from("/tmp/loongclaw/loongclaw.config.yaml")),
            runtime_logs_dir: PathBuf::from("/tmp/loongclaw/runtime/logs"),
            service_env,
        }
    }
    ...
    fn test_render_linux_unit_contains_expected_fields() {
        ...
        assert!(unit.contains("ExecStart=/usr/local/bin/loongclaw start"));
        assert!(unit.contains("Environment=LOONGCLAW_GATEWAY=1"));
        assert!(unit.contains("LOONGCLAW_CONFIG=/tmp/loongclaw/loongclaw.config.yaml"));
    }
    ...
    fn test_render_macos_plist_contains_required_fields() {
        ...
        assert!(plist.contains("LOONGCLAW_GATEWAY"));
        assert!(plist.contains("LOONGCLAW_CONFIG"));
        assert!(normalized.contains("/tmp/loongclaw/runtime/logs/loongclaw-gateway.log"));
        assert!(normalized.contains("/tmp/loongclaw/runtime/logs/loongclaw-gateway.error.log"));
    }
    ...
    fn test_resolve_runtime_logs_dir_fallback() {
        let dir = resolve_runtime_logs_dir(Path::new("/tmp/loongclaw"));
        assert!(
            dir.ends_with("runtime/logs") || dir.ends_with("loongclaw.data/runtime/logs"),
            "unexpected logs dir: {}",
            dir.display()
        );
    }
