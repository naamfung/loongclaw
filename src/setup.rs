...
const CONFIG_BACKUP_DIR_NAME: &str = "loongclaw.config.backups";
...
fn parse_accounts_json_value(
    raw: &str,
    field_key: &str,
) -> Result<Option<serde_json::Map<String, serde_json::Value>>, LoongClawError> {
    let trimmed = raw.trim();
    if trimmed.is_empty() {
        return Ok(None);
    }
    let parsed: serde_json::Value = serde_json::from_str(trimmed).map_err(|e| {
        LoongClawError::Config(format!("{field_key} must be valid JSON object: {e}"))
    })?;
...
fn parse_boolish(value: &str, default_if_empty: bool) -> Result<bool, LoongClawError> {
    ...
    match raw.as_str() {
        "true" | "1" | "yes" => Ok(true),
        "false" | "0" | "no" => Ok(false),
        _ => Err(LoongClawError::Config(format!(
...
fn parse_bot_count(value: &str, field_key: &str) -> Result<usize, LoongClawError> {
...
fn parse_i64_list_field(value: &str, field_key: &str) -> Result<Vec<i64>, LoongClawError> {
    let trimmed = value.trim();
    if trimmed.is_empty() {
        return Ok(Vec::new());
    }

    let parse_item = |raw: &str| -> Result<i64, LoongClawError> {
            ...
            match item {
                serde_json::Value::Number(n) => {
                    let id = n.as_i64().ok_or_else(|| {
                        LoongClawError::Config(format!("{field_key} contains non-integer number"))
                    })?;
                    out.push(id);
                }
...
fn default_data_dir_for_setup() -> String {
    std::env::var_os("HOME")
        ...
        .map(|p| p.join(".loongclaw"))
        .unwrap_or_else(|| std::path::PathBuf::from(".loongclaw"))
        .to_string_lossy()
        .to_string()
}
...
impl SetupApp {
    ...
    /// Load existing config values from loongclaw.config.yaml/.yml.
    fn load_existing_config() -> HashMap<String, String> {
        let yaml_path = if Path::new("./loongclaw.config.yaml").exists() {
            Some("./loongclaw.config.yaml")
        } else if Path::new("./loongclaw.config.yml").exists() {
            Some("./loongclaw.config.yml")
        } else {
            None
        };
    ...
    fn telegram_slot_accounts_from_fields(
        &self,
    ) -> Result<serde_json::Map<String, serde_json::Value>, LoongClawError> {
        let mut out = serde_json::Map::new();
    ...
    fn validate_local(&self) -> Result<(), LoongClawError> {
        for field in &self.fields {
            if self.is_field_required(field) && field.value.trim().is_empty() {
                return Err(LoongClawError::Config(format!("{} is required", field.key)));
            }
        }
        ...
        if self.channel_enabled("telegram") {
            ...
            if self.telegram_bot_count() > 1 && telegram_slot_accounts.is_empty() {
                return Err(LoongClawError::Config(
        ...
        if provider.is_empty() {
            return Err(LoongClawError::Config("LLM_PROVIDER is required".into()));
        }
        ...
        tz.parse::<chrono_tz::Tz>()
            .map_err(|_| LoongClawError::Config(format!("Invalid TIMEZONE: {tz}")))?;
        ...
        if !memory_token_budget_raw.is_empty() {
            let memory_token_budget = memory_token_budget_raw.parse::<usize>().map_err(|_| {
                LoongClawError::Config("MEMORY_TOKEN_BUDGET must be a positive integer".into())
            })?;
        ...
        if !embedding_dim_raw.is_empty() {
            let embedding_dim = embedding_dim_raw.parse::<usize>().map_err(|_| {
                LoongClawError::Config("EMBEDDING_DIM must be a positive integer".into())
            })?;
            if embedding_dim == 0 {
                return Err(LoongClawError::Config(
    ...
    fn validate_online(&self) -> Result<Vec<String>, LoongClawError> {
        ...
        .join()
        .map_err(|_| LoongClawError::Config("Validation thread panicked".into()))?
    }
...
) -> Result<Vec<String>, LoongClawError> {
...
fn create_config_backup(path: &Path) -> Result<Option<String>, LoongClawError> {
    if !path.exists() {
        return Ok(None);
    }
    let file_name = path
        .file_name()
        .and_then(|n| n.to_str())
        .unwrap_or("loongclaw.config.yaml");
...
fn save_config_yaml(
...
) -> Result<Option<String>, LoongClawError> {
    ...
    yaml.push_str("# LoongClaw configuration\n\n");
    yaml.push_str(
    ...
    if channel_selected("telegram") || telegram_present {
        ...
        if let Some(accounts) = &telegram_accounts {
            ...
            let yaml_accounts = serde_yaml::to_value(serde_json::Value::Object(accounts.clone()))
                .map_err(|e| {
                LoongClawError::Config(format!("Failed to render Telegram multi-bot accounts: {e}"))
            })?;
    ...
    if channel_selected("discord") || discord_present {
        ...
        if let Some(accounts) = &discord_accounts {
            ...
            let yaml_accounts = serde_yaml::to_value(serde_json::Value::Object(accounts.clone()))
                .map_err(|e| {
                LoongClawError::Config(format!("Failed to render DISCORD_ACCOUNTS_JSON: {e}"))
            })?;
            append_yaml_value(&mut yaml, 6, &yaml_accounts);
    ...
    for (ch, include) in &dynamic_channel_include {
        ...
        if has_accounts {
            ...
            let yaml_accounts = serde_yaml::to_value(serde_json::Value::Object(accounts_map))
                .map_err(|e| {
                    LoongClawError::Config(format!("Failed to render {} accounts: {e}", ch.name))
                })?;
            append_yaml_value(&mut yaml, 6, &yaml_accounts);
        }
    }
...
fn run_with_spinner<T, F>(
    ...
    label: &str,
    work: F,
) -> Result<T, LoongClawError>
where
    T: Send + 'static,
    F: FnOnce() -> Result<T, LoongClawError> + Send + 'static,
{
    let (tx, rx) = mpsc::channel::<Result<T, LoongClawError>>();
    std::thread::spawn(move || {
        let _ = tx.send(work());
    });
...
fn try_save(terminal: &mut DefaultTerminal, app: &mut SetupApp) -> Result<(), LoongClawError> {
    ...
    let backup = match run_with_spinner(
        terminal,
        app,
        "Saving (3/3): writing loongclaw.config.yaml",
        move || save_config_yaml(Path::new("loongclaw.config.yaml"), &values),
    ) {
        Ok(v) => v,
        Err(e) => {
            app.status = format!("Cannot save: {e}");
    ...
    app.status = "Saved loongclaw.config.yaml".into();
    app.completed = true;
    Ok(())
}
...
fn try_save_skip_online(
    ...
    let backup = match run_with_spinner(
        terminal,
        app,
        "Saving (2/2): writing loongclaw.config.yaml",
        move || save_config_yaml(Path::new("loongclaw.config.yaml"), &values),
    ) {
        Ok(v) => v,
        Err(e) => {
    ...
    app.status = "Saved loongclaw.config.yaml (online validation skipped)".into();
    app.completed = true;
    Ok(())
}

fn run_wizard(mut terminal: DefaultTerminal) -> Result<bool, LoongClawError> {
...
pub fn run_setup_wizard() -> Result<bool, LoongClawError> {
    enable_raw_mode()?;
...
pub fn enable_sandbox_in_config() -> Result<String, LoongClawError> {
    let Some(path) = Config::resolve_config_path()? else {
        return Err(LoongClawError::Config(
            "No loongclaw.config.yaml found. Run `loongclaw setup` first.".to_string(),
        ));
    };
...
mod tests {
    ...
    fn test_save_config_yaml_respects_high_risk_confirmation_toggle() {
        let yaml_path = std::env::temp_dir().join(format!(
            "loongclaw_setup_high_risk_confirm_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_save_config_yaml_uses_accounts_json_for_telegram_and_discord() {
        let yaml_path = std::env::temp_dir().join(format!(
            "loongclaw_setup_accounts_json_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_enable_sandbox_in_config_updates_mode() {
        ...
        let path = std::env::temp_dir().join(format!(
            "loongclaw_setup_enable_sandbox_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
        std::fs::write(
            &path,
...
"#,
        ...
        .unwrap();
        std::env::set_var("LOONGCLAW_CONFIG", &path);
    ...
    fn test_save_config_yaml_disables_web_when_not_selected() {
        let yaml_path = std::env::temp_dir().join(format!(
            "loongclaw_setup_web_toggle_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_save_config_yaml_keeps_telegram_disabled_with_credentials() {
        let yaml_path = std::env::temp_dir().join(format!(
            "loongclaw_setup_telegram_disabled_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_save_config_yaml_keeps_dynamic_channel_disabled_when_not_selected() {
        let yaml_path = std::env::temp_dir().join(format!(
            "loongclaw_setup_dynamic_skip_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_save_config_yaml_includes_dynamic_channel_when_selected() {
        let yaml_path = std::env::temp_dir().join(format!(
            "loongclaw_setup_dynamic_include_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_save_config_yaml_writes_feishu_topic_mode_as_bool() {
        let yaml_path = std::env::temp_dir().join(format!(
            "loongclaw_setup_feishu_topic_mode_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_save_config_yaml_writes_feishu_show_progress_as_bool() {
        let yaml_path = std::env::temp_dir().join(format!(
            "loongclaw_setup_feishu_show_progress_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_save_config_yaml_uses_telegram_slot_fields_when_multibot_enabled() {
        let yaml_path = std::env::temp_dir().join(format!(
            "loongclaw_setup_tg_slots_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
        let mut values = HashMap::new();
