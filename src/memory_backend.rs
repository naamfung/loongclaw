...
use loongclaw_storage::db::{call_blocking, Database, Memory};
...
impl MemoryBackend {
    ...
    pub async fn get_memory_by_id(&self, id: i64) -> Result<Option<Memory>, LoongClawError> {
        if let Some(mcp) = &self.mcp {
            let payload = serde_json::json!({
                "op": "get",
                "id": id,
    ...
    pub async fn archive_memory(&self, id: i64) -> Result<bool, LoongClawError> {
        if let Some(mcp) = &self.mcp {
            let payload = serde_json::json!({
                "op": "archive",
                "id": id,
