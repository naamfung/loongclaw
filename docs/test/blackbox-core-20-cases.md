...
本文档用于手工黑盒回归测试 LoongClaw 核心能力。每个用例都给出可直接执行的步骤、发送消息和验收标准。
...
4. 如需验证 MCP 相关用例，确保 `mcp.json` 或 `mcp.d/*.json` 已配置并可用。
...
1. 请求 agent 写入并读取文件。
- 发送消息:
`请用工具在当前工作目录写入文件 tc07.txt，内容是 hello-loongclaw，然后再读出来返回。`
- 预期:
1. 工具调用成功。
2. 返回内容包含 `hello-loongclaw`。
- 记录:
1. 结果：
2. 备注：
