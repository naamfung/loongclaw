...
- Enable quickly: `loongclaw setup --enable-sandbox`.
- Verify readiness: `loongclaw doctor sandbox`.
...
- Sandbox mount validation:
  ...
  - optional external mount allowlist (`~/.loongclaw/sandbox-mount-allowlist.txt`)
- File path guard:
  - sensitive path deny list
  - symlink validation on existing path prefix
  - optional external path allowlist (`~/.loongclaw/sandbox-path-allowlist.txt`)
