...
This guide describes an indirect integration pattern: run a HAPI bridge as an external MCP sidecar and let LoongClaw consume it through MCP tools.

## Why this pattern

- No deep embedding into `agent_engine`.
- Bridge lifecycle and failure domain stay outside LoongClaw core.
...
2. LoongClaw can reach the endpoint.
...
4. Start bridge first, then start LoongClaw:
