...
- List hooks: `loongclaw hooks list`
- Inspect hook: `loongclaw hooks info <name>`
- Disable bad hook quickly: `loongclaw hooks disable <name>`
...
Use this pattern when you want remote terminal/session capability via an external bridge, without embedding terminal runtime logic into LoongClaw core.
...
3. Start the bridge service separately, then start LoongClaw.
...
If any configured MCP server exposes both `memory_query` and `memory_upsert`, LoongClaw enables MCP-first structured-memory operations:
...
- on MCP call failure, timeout, or invalid/unknown response shape, LoongClaw falls back to built-in SQLite memory automatically
