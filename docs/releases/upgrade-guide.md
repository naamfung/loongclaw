...
- hook runtime and `loongclaw hooks` CLI
...
1. Backup SQLite database (`loongclaw.db`).
2. Record current config (`loongclaw.config.yaml`).
...
2. Verify discovery with `loongclaw hooks list`.
3. Enable one-by-one with `loongclaw hooks enable <name>`.
...
3. Restore pre-upgrade `loongclaw.db` backup.
4. Restore previous `loongclaw.config.yaml`.
