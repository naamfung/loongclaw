...
<p align="center">
  <img src="docs/assets/readme/memory-architecture.svg" alt="LoongClaw 记忆架构图" width="92%" />
</p>

LoongClaw 通过 `AGENTS.md` 文件维护持久化记忆：
...
另外，LoongClaw 也会把结构化记忆写入 SQLite（`memories` 表）：
...
<p align="center">
  <img src="docs/assets/readme/skills-lifecycle.svg" alt="LoongClaw 技能生命周期图" width="92%" />
</p>

LoongClaw 支持 [Anthropic Agent Skills](https://github.com/anthropics/skills) 标准。技能是为特定任务提供专业能力的模块化包。
...
这样可以在不改 LoongClaw 核心 agent loop 的情况下，通过外部桥接服务扩展远程终端能力。

详细操作可见：`docs/operations/hapi-bridge.md`。

### 在 macOS 上接入 Peekaboo MCP（桌面自动化）

[Peekaboo](https://github.com/steipete/Peekaboo) 是一个 macOS 桌面自动化 MCP server。LoongClaw 可通过 `stdio` 直接接入（无需修改运行时代码）。
...
LoongClaw 也可以通过 `stdio` 接入 Windows 桌面自动化 MCP server：
...
注意：部分 Windows MCP 项目只提供 `sse` transport。LoongClaw 当前仅直接支持 `stdio` 与 `streamable_http`，SSE-only 服务需先通过协议桥接后再接入。

## 计划与执行

<p align="center">
  <img src="docs/assets/readme/plan-execute.svg" alt="LoongClaw 计划执行流程图" width="92%" />
</p>
...
<p align="center">
  <img src="docs/assets/readme/task-scheduler.svg" alt="LoongClaw 定时任务流程图" width="92%" />
</p>
...
当 `web_enabled: true` 时，LoongClaw 会启动本地 Web UI（默认 `http://127.0.0.1:10961`）。
...
> **新功能：** 现在支持交互式问答配置（`loongclaw config`），并且在 `start` 时若缺少必需配置会自动进入配置流程。
...
2. 发送 `/newbot`
3. 输入机器人的显示名称（例如 `My LoongClaw`）
4. 输入用户名（必须以 `bot` 结尾，例如 `my_loongclaw_bot`）
...
- 安全写入 `loongclaw.config.yaml`（自动备份）
...
对于 `openai-codex`：你可以先运行 `codex login`，LoongClaw 会读取 `~/.codex/auth.json`（或 `$CODEX_HOME/auth.json`）里的 OAuth 凭据。也可以在使用 OpenAI 兼容中转端点时配置 `api_key`。默认 base URL 是 `https://chatgpt.com/backend-api`。

如果你更喜欢手工配置，也可以直接写 `loongclaw.config.yaml`：
...
- 日志按小时分片：`loongclaw-YYYY-MM-DD-HH.log`
- 超过 30 天的日志会自动删除

## 配置项

所有配置都在 `loongclaw.config.yaml` 中。
...
| `data_dir` | 否 | `~/.loongclaw` | 数据根目录（运行时数据在 `data_dir/runtime`，技能在 `data_dir/skills`） |
| `working_dir` | 否 | `~/.loongclaw/working_dir` | 工具默认工作目录；`bash/read_file/write_file/edit_file/glob/grep` 的相对路径都以此为基准 |
...
| `channels.irc.real_name` | 否 | `"LoongClaw"` | IRC real name（USER 命令字段） |
...
- 如果未配置，则默认使用 `data_dir=~/.loongclaw`、`skills_dir=<data_dir>/skills`、`working_dir=~/.loongclaw/working_dir`。
...
- 可选加固：
  - `~/.loongclaw/sandbox-mount-allowlist.txt`：沙箱挂载路径白名单。
  - `~/.loongclaw/sandbox-path-allowlist.txt`：文件工具路径白名单。
...
LoongClaw 的核心智能体循环是渠道无关的。新增平台时，重点是实现适配器层：
